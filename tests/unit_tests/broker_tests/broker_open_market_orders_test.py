...
def test_open_long_single(
    test_account: Account, test_broker: Broker, trades_log: List[Trade]
):
    ...
    assert trades_log[0].trade_type == TradeType.OPEN
    assert trades_log[0].position_type == PositionType.LONG
    assert trades_log[0].open_price == pytest.approx(100.0, abs=0.01)
    assert trades_log[0].open_size == 1
    assert trades_log[0].close_price is None
    assert trades_log[0].close_size is None
    assert trades_log[0].market_order is True
...
def test_open_long_multiple_in_single_process(
    ...
    assert trades_log[0].trade_type == TradeType.OPEN
    assert trades_log[0].position_type == PositionType.LONG
    assert trades_log[0].open_price == pytest.approx(25.0, abs=0.01)
    assert trades_log[0].open_size == 1
    assert trades_log[0].close_price is None
    assert trades_log[0].close_size is None
    assert trades_log[0].market_order is True
    assert trades_log[1].trade_type == TradeType.OPEN
    assert trades_log[1].position_type == PositionType.LONG
    assert trades_log[1].open_price == pytest.approx(25.0, abs=0.01)
    assert trades_log[1].open_size == 2
    assert trades_log[1].close_price is None
    assert trades_log[1].close_size is None
...
def test_open_long_multiple_in_multiple_processes(
    ...
    assert trades_log[0].trade_type == TradeType.OPEN
    assert trades_log[0].position_type == PositionType.LONG
    assert trades_log[0].open_price == pytest.approx(50.0, abs=0.01)
    assert trades_log[0].open_size == 1
    assert trades_log[0].close_price is None
    assert trades_log[0].close_size is None
    assert trades_log[0].market_order is True
...
def test_open_short_single(
    ...
    assert len(trades_log) == 1
    assert trades_log[0].trade_type == TradeType.OPEN
    assert trades_log[0].position_type == PositionType.SHORT
    assert trades_log[0].open_price == pytest.approx(100.0, abs=0.01)
    assert trades_log[0].open_size == 1
    assert trades_log[0].close_price is None
    assert trades_log[0].close_size is None
...
def test_open_long_with_fixed_spread(
    test_account: Account, test_broker: Broker, trades_log: List[Trade]
):
    open_order = OpenOrder(
        size=1,
    ...
    assert trades_log[0].trade_type == TradeType.OPEN
    assert trades_log[0].position_type == PositionType.LONG
    assert trades_log[0].open_price == pytest.approx(92.2, abs=0.01)
    assert trades_log[0].open_size == 1
    assert trades_log[0].close_price is None
    assert trades_log[0].close_size is None
    assert trades_log[0].market_order is True
...
def test_open_short_with_fixed_spread(
    test_account: Account, test_broker: Broker, trades_log: List[Trade]
):
    open_order = OpenOrder(
        size=1,
    ...
    assert trades_log[0].trade_type == TradeType.OPEN
    assert trades_log[0].position_type == PositionType.SHORT
    assert trades_log[0].open_price == pytest.approx(87.8, abs=0.01)
    assert trades_log[0].open_size == 1
    assert trades_log[0].close_price is None
    assert trades_log[0].close_size is None
    assert trades_log[0].market_order is True
...
def test_open_long_relative_commision(
    test_account: Account, test_broker: Broker, trades_log: List[Trade]
):
    open_order = OpenOrder(
        size=1,
    ...
    assert trades_log[0].trade_type == TradeType.OPEN
    assert trades_log[0].position_type == PositionType.LONG
    assert trades_log[0].open_price == pytest.approx(90.0, abs=0.01)
    assert trades_log[0].open_size == 1
    assert trades_log[0].close_price is None
    assert trades_log[0].close_size is None
    assert trades_log[0].market_order is True
...
def test_open_long_commision_and_spread(
    test_account: Account, test_broker: Broker, trades_log: List[Trade]
):
    open_order = OpenOrder(
        size=1,
...
def test_open_long_minimum_commision_less(
    test_account: Account, test_broker: Broker, trades_log: List[Trade]
):
    open_order = OpenOrder(
        size=1,
...
def test_open_long_fixed_commision(
    test_account: Account, test_broker: Broker, trades_log: List[Trade]
):
    open_order = OpenOrder(
        size=1,
...
@pytest.mark.parametrize(
    "market_data, spread_rate, spread_type",
    [([(None, 90.0, None, None, None, None)], 0.02, SpreadType.RELATIVE)],
)
def test_open_long_with_relative_spread(
    test_account: Account, test_broker: Broker, trades_log: List[Trade]
):
    open_order = OpenOrder(
        size=1,
    ...
    assert trades_log[0].trade_type == TradeType.OPEN
    assert trades_log[0].position_type == PositionType.LONG
    assert trades_log[0].open_price == pytest.approx(91.8, abs=0.01)
    assert trades_log[0].open_size == 1
    assert trades_log[0].close_price is None
    assert trades_log[0].close_size is None
    assert trades_log[0].market_order is True
    ...
    assert test_broker.get_positions()[0].position_type == PositionType.LONG
    assert test_broker.get_positions()[0].size == 1
    assert test_broker.get_positions()[0].open_price == pytest.approx(91.8, abs=0.01)
    assert test_broker.get_positions()[0].stop_loss is None
    assert test_broker.get_positions()[0].take_profit is None
...
def test_open_short_with_relative_spread(
    test_account: Account, test_broker: Broker, trades_log: List[Trade]
):
    open_order = OpenOrder(
        size=1,
    ...
    assert trades_log[0].trade_type == TradeType.OPEN
    assert trades_log[0].position_type == PositionType.SHORT
    assert trades_log[0].open_price == pytest.approx(88.2, abs=0.01)
    assert trades_log[0].open_size == 1
    assert trades_log[0].close_price is None
    assert trades_log[0].close_size is None
    assert trades_log[0].market_order is True
    ...
    assert test_broker.get_positions()[0].position_type == PositionType.SHORT
    assert test_broker.get_positions()[0].size == 1
    assert test_broker.get_positions()[0].open_price == pytest.approx(88.2, abs=0.01)
    assert test_broker.get_positions()[0].stop_loss is None
    assert test_broker.get_positions()[0].take_profit is None

    assert test_broker.get_assets_value() == pytest.approx(86.4, abs=0.01)
    assert test_account.current_money == pytest.approx(11.8, abs=0.01)
