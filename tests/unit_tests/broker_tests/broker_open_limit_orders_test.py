...
@pytest.mark.parametrize(
    "market_data, commission", [([(None, 100.0, 100.0, 90.0, 90.0, None)], 0.02)]
)
def test_open_long_commission(
    test_data: Data, test_account: Account, test_broker: Broker
):
    open_order = OpenOrder(size=1, position_type=PositionType.LONG, limit_price=90.0)
    test_broker.process_new_orders([open_order])
...
@pytest.mark.parametrize(
    "market_data, commission", [([(None, 110.0, 110.0, 99.0, 99.0, None)], 0.02)]
)
def test_open_long_commission_not_enough_money(
    test_data: Data, test_account: Account, test_broker: Broker
):
...
@pytest.mark.parametrize(
    ...
    [([(None, 100.0, 100.0, 85.0, 85.0, None)], 0.02, 2.2)],
)
...
def test_open_short_commission(
    test_data: Data, test_account: Account, test_broker: Broker
):
...
@pytest.mark.parametrize(
    "market_data, commission", [([(None, 90.0, 99.0, 90.0, 99.0, None)], 0.02)]
)
def test_open_short_commission_not_enough_money(
    test_data: Data, test_account: Account, test_broker: Broker
):
...
@pytest.mark.parametrize(
    "market_data, commission, spread",
    [([(None, 80.0, 95.0, 80.0, 95.0, None)], 0.02, 2.2)],
)
